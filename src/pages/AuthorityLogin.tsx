...
const AuthorityLogin = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try 
    {
      ...
      if (role !== "authority") {
        await auth.signOut();
        toast({ title: t("accessDenied"), description: t("notAuthority"), variant: "destructive" });
        return;
      }
      toast({ title: t("loginSuccessful"), description: t("welcomeAuthority") });
      const redirectUrl = sessionStorage.getItem("redirectUrl");
      sessionStorage.removeItem("redirectUrl");
      navigate(redirectUrl || "/authority");
    } catch (err: any) {
